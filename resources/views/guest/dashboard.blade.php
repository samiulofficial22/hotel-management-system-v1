...
@else
    ...
    <div class="text-center mb-4 py-4">
        <a href="{{ route('guest.profile') }}" class="d-inline-block text-decoration-none">
            <img src="{{ auth()->user()->profile_pic_url }}" alt="{{ $guest->full_name }}" class="rounded-circle shadow-sm border border-3 border-primary" width="120" height="120" style="object-fit: cover;">
        </a>
        <h2 class="h4 mt-3 mb-1">{{ __('Welcome') }}, {{ $guest->full_name }}</h2>
        <p class="text-muted mb-2">{{ $guest->email ?? $guest->phone ?? '' }}</p>
        <a href="{{ route('guest.profile') }}" class="btn btn-outline-primary btn-sm">{{ __('My profile') }}</a>
    </div>

    {{-- Book a room --}}
    <div class="mb-4">
        <a href="{{ route('booking.request') }}" class="card text-decoration-none text-dark border-primary shadow-sm hover-shadow">
            <div class="card-body d-flex align-items-center gap-3 py-4">
                ...
                <div class="flex-grow-1">
                    ...
                    <p class="text-muted small mb-0">{{ __('Request a new room booking. We will confirm after review.') }}</p>
                </div>
