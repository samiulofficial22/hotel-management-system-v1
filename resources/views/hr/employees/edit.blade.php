    ...
    <div class="card mb-4 shadow-sm">
        ...
        <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $employee->email ?? $employee->user?->email) }}" id="emp-email">
            ...
            <div class="mb-3">
                ...
                <input type="password" name="password" class="form-control @error('password') is-invalid @enderror" autocomplete="new-password" minlength="8" placeholder="{{ __('Set new password for login') }}">
                <small class="text-muted">{{ __('Leave blank to keep current. Fill to change password or create login if not linked. Min 8 characters.') }}</small>
            ...
            <div class="mb-3">
                ...
                <div class="d-flex align-items-center gap-3">
                    ...
                    <small class="text-muted">{{ __('Managed in user profile.') }} @if($employee->user_id)<a href="{{ route('users.edit', $employee->user_id) }}">{{ __('Edit user profile') }}</a>@endif</small>
                </div>
            </div>
            <div class="mb-3">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="employment_type" class="form-control @error('employment_type') is-invalid @enderror" value="{{ old('employment_type', $employee->employment_type) }}" placeholder="e.g. Full-time" maxlength="50">
                    @error('employment_type')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div class="col-md-6 mb-3">
                    <label class="form-label">{{ __('Salary') }}</label>
                    <input type="number" name="salary" class="form-control @error('salary') is-invalid @enderror" value="{{ old('salary', $employee->salary ?? $employee->base_salary) }}" step="0.01" min="0" placeholder="0">
                    @error('salary')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
            </div>
            <div class="mb-0">
                <label class="form-label">{{ __('Shift') }}</label>
                <input type="text" name="shift" class="form-control @error('shift') is-invalid @enderror" value="{{ old('shift', $employee->shift) }}" placeholder="e.g. Morning" maxlength="50">
